...
library(DT)
newdata=read.csv("https://raw.githubusercontent.com/jessmahoney12/dataforIDDAP/master/mdata.csv",stringsAsFactors=FALSE, check.names = FALSE)

plotdata=read.csv("https://raw.githubusercontent.com/jessmahoney12/dataforIDDAP/master/mdata.csv",stringsAsFactors= FALSE, check.names = TRUE)
...
inputfile<- filter(inputfile, (as.integer(input$OPATad[2]-as.Date.character(`OPAT Start Date`,"%m/%d/%Y"))) > 0)
  ...
  if("1" %in% input$riskofre==FALSE )
    inputfile<- filter(inputfile,`High Risk of Readmission`==c("Y","YES","YEs","Yes","y","yE","yES","yes"))
  if("2" %in% input$riskofre==FALSE)
    inputfile<- filter(inputfile,`High Risk of Readmission`==c("N","NO","No","nO","no"))
  ...
  if("1" %in% input$resis==FALSE )
    inputfile<- filter(inputfile,`Resistant Organism Y/N`==c("Y","YES","YEs","Yes","y","yE","yES","yes"))
  if("2" %in% input$resis==FALSE )
    inputfile<- filter(inputfile,`Resistant Organism Y/N`==c("N","NO","No","nO","no"))
  if(is.null(input$resis))
    inputfile <- "please select a resistance"

  # Gender Checkbox Group conditionals
  if("Male" %in% input$MF==FALSE )
    inputfile<- filter(inputfile,`Gender (M/F)`==c("F","f"))
  if("Female" %in% input$MF==FALSE )
    inputfile<- filter(inputfile,`Gender (M/F)`==c("M","m"))
  if(is.null(input$resis))
...
inputfile<- filter(inputfile, (as.integer(input$OPATad[2]-as.Date.character(`OPAT Start Date`,"%m/%d/%Y"))) > 0)
  ...
  if(is.null(input$AB))
    inputfile <- "please select an antibiotic type"
ggplot(inputfile,aes(x=Antibiotic))+geom_bar(fill=brewer.pal(7,"Set2"))+ylab("Number of Patients Taking Antibiotic")
})
```
